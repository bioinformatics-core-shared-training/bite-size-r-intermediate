...
clinicalData <- gather(clinicalData, Treatment, Value, -Subject)
clinicalData
```
...
```{r}
clinicalReplicateData <- separate(clinicalData, Treatment, into = c("Treatment", "Replicate"))
clinicalReplicateData
```
...
```{r}
ggplot(clinicalReplicateData, mapping = aes(x = Replicate, y = Value)) +
  geom_boxplot() +
...
```{r}
spread(clinicalData, key = Treatment, value = Value)
```
