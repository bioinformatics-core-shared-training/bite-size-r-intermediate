...
We then use the geom_point function is used to specify a point plot, a scatter plot of Height values on the x-axis versus Weight values on the y values.

```{r ggplot_only}
pcPlot <- ggplot(data=patients_clean) #+ aes(x=Height,y=Weight) #+ geom_point()
class(pcPlot)
head(pcPlot$data)
#pcPlot$mapping # inspect mappings
#pcPlot$theme
#pcPlot$layers
```
...
```{r, line_simple}
p = ggplot(data=patients_clean, mapping=aes(x=Height,y=Weight)) # x and y are numeric
#p = ggplot(data=patients_clean, mapping=aes(x=Sex))             # factor (categorical) for x
#p = ggplot(data=patients_clean, mapping=aes(x=Sex,y=Weight))    # factor for x and numeric for y
...
[Link_to_exercises](http://mrccsc.github.io/exercise_1_exercises.html)

[Link_to_exercises_with_images](http://mrccsc.github.io/exercise_1_exercises_images.html)

[Link_to_Rmarkdown template](http://mrccsc.github.io/exercise_1_empty.Rmd)

[Link_to_answers](http://mrccsc.github.io/exercise_1_answers.html)


# Adding titles for plot and labels.

So far no plot titles have been specified. Plot titles can be specified using the ggtitle and xlab/ylab functions.
...
pcPlot = ggplot(patients_clean, aes(x=Height,y=Weight,colour=Pet)) + geom_point(size=4)
...
An example using scale_colour_gradient below sets the low and high end colours to White and Red respectively. Similarly we can use the scale_colour_gradient2 function which allows for the specification of a midpoint value and its associated colour.
...
The stat_smooth() function can be used to fit a line to the data being displayed. Other methods available include lm, glm,gam,rlm.
...
If colour by Sex is an aesthetic mapping then two smooth lines are drawn, one for each sex. This behaviour can be overridden by specifying an aes within the stat_smooth() function and setting inherit.aes to FALSE.
...
[Link_to_exercises](http://mrccsc.github.io/exercise_2_exercises.html)

[Link_to_exercises_with_images](http://mrccsc.github.io/exercise_2_exercises_images.html)

[Link_to_Rmarkdown template](http://mrccsc.github.io/exercise_2_empty.Rmd)

[Link_to_answers](http://mrccsc.github.io/exercise_2_answers.html)
