...
```{r}
patients <- head(read.delim("patient-data.txt"))
```
...
******
- [Rmarkdown Template](3.workflow-exercise.Rmd)
- Take the steps used to clean the patients dataset and calculate BMI (see template in `workflow-exercise.Rmd` for the code)
```{r}
...
The **`filter`** verb is used to choose rows from the data frame. The criteria we use to choose can use the comparisons `==`, `>`, `<`, `!=`
...
```{r}
grep("Jo", patients_clean$Name)
grepl("Jo",patients_clean$Name)
filter(patients_clean, grepl("Jo",Name))
```
...
Females with height more than 160

```{r}
filter(patients_clean, Sex == "Female", Height >160)
```
...
******
## Exercise: filter
See [3.workflows-exercise.Rmd](Markdown template)
Use `filter` to print the following subsets of the dataset

  - Choose the patients that are still alive
...
- Recall that functions `write.table`, `write.csv` are used to write a data frame to a tab-delimited or csv file. Because these functions accept a data frame as their first argument, they fit easily into the pipeline framework as the last step.
...
```{r}
arrange(patients_clean, desc(Grade_Level), Count) %>% select(ID:Sex,Count)
```

`Grade_Level`, `Sex` and then `Count`

```{r}
arrange(patients_clean, desc(Grade_Level), Sex, Count) %>% select(ID:Sex,Count)
```
...
- Best to build the pipeline step-by-step
