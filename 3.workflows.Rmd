...
> apply the `tbl_df` function to the result of reading the file `patient-data.txt`
...
```{r echo=FALSE}
mutate(patients_clean, Weight = as.numeric(str_replace_all(Weight,"kg",""))) %>% 
  mutate(BMI = (Weight/(Height/100)^2), Overweight = BMI > 25) %>% 
  mutate(Smokes = str_replace_all(Smokes, "Yes", TRUE)) -> patients_clean
...
```{r}
filter(patients_clean, Pet %in% c("CAT", "Cat")) %>% select(ID,Name, Race,Age,Pet)
```
...
******
  ...
  - Choose the Smokers with Age over 30
  - Choose the patients that are still alive
  - Choose the patients who do NOT own dogs
...
- Choose the candidates (overweight smokers)
...
e.g. `Age` and then `Count`

```{r}
arrange(patients_clean, desc(Age), Count) %>% select(ID:Sex,Count)
```

`Age`, `Sex` and then `Count`

```{r}
arrange(patients_clean, desc(Age), Sex, Count) %>% select(ID:Sex,Count)
```
