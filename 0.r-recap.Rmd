...
At a basic level, we can use R as a calculator to compute simple sums with the `+`, `-`, `*` (for multiplication) and `/` (for division). 
...
The answer is displayed a the console with a `[1]` in front of it. The `1` inside the square brackets is a place-holder to signify how many values were returned (in this case only one). We will talk about dealing with lists of numbers shortly...

R respects the [BODMAS](https://en.wikipedia.org/wiki/Order_of_operations#Mnemonics) system to decide which operations should be performed first in an expression involving multiple operators. If in doubt, brackets can always be used.
...
R is capable of more complicated arithmetic such as trigonometry and logarithms; like you would find on a scientific calculator. Of course, R also has a plethora of statistical operations as we will see.
...
The normal distribution is defined by a mean and standard deviation. However, in the above example we didn't tell R what mean and standard deviation we wanted. So how does R know what to do? All arguments to a function and their default values are listed in the help page
...
Testing for equality can be done using `==`. This will only give `TRUE` for entries that are *exactly* the same as the test string. N.B. For partial matches, regular expressions can be used.
...
It is important to realise that base graphics in R uses a *"canvas model"* to create graphics. That said, we can only overlay extra information on-top of an exising plot and cannot "undo" what is already drawn.
...
We can start by plotting the life expectancy of the European countries.
...
The `points` function can put extra points on an existing plot. 
...
The problem here is that the initial limits of the y axis were defined using the life expectancy range of the European data. We can only add points to the existing plotting window, so any African countries with life expectancy outside this will not get displayed.
...
```{r}
model <- aov(gapminder$gdpPercap~gapminder$continent)
anova(model)
```
