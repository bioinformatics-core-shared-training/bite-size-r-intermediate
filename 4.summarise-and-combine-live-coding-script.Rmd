...
Here, we've had to specify the '.' character to represent the columns that we're deviding by 100.
...
While the `summarise` function is useful on its own, it becomes really powerful when applied to groups of observations within a dataset. For example, suppose we want to compute the mean height for each of the race in the `patients` dataset. We could take each race in turn and `filter` the data frame to only contain rows for a given race, then apply `summarise`, but that would be somewhat cumbersome. In `dplyr`, the `group_by` function allows this to be done in one simple step.
...
patients %>% group_by( Race) %>% summarise( mean(Height))
```
...
patients %>% group_by( Race) %>% summarise( n(), mean(Height), sd(Height))
...
patients %>% group_by(Race) %>% summarise( N=n())

patients %>% group_by( Race) %>% count()
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
patients %>% group_by( Race) %>% summarise_at( vars(Height, Weight), funs(mean))

```

```{r echo=TRUE, message=FALSE, warning=FALSE}
 patients %>% group_by( Race) %>% 
  summarise_at( vars(Height, Weight), funs(mean)) %>% 
  gather( measure, mean, -Race) %>% 
  ggplot(aes(x = Race, y = mean, color = measure, group=measure)) + geom_point() + geom_line()
...
patients %>% group_by(Race) %>% summarise(correlation = cor(Height, Weight))
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
patients_grouped <- patients %>%  group_by( Race)
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
patients_ungroped <- ungroup(patients_grouped)
class(patients_ungroped)
groups(patients_ungroped)
