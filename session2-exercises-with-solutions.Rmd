---
title: "Session 2 exercises - Solutions"
...
Currently the columns are `Placebo.1`, `Placebo.2`...`Drug1.1` etc., however,
"Placebo.." and "Drug.." are values not variables. Really there should be two
variables, one called something like **Treatment** containing values of
'Placebo..', 'Drug..', and another called something like **Value** or
**Measure** with the numbers. Possibly, the Treatment values are "Placebo",
"Drug1" and "Drug2" and the number after the '.' indicates a replicate, but we
don't know this for sure.

3. Transform the data into a tidy form using the `gather` function from the
   `tidyr` package.
