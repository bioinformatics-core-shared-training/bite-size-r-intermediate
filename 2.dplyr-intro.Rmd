...
These data are found in the file [tidyr-example.txt](tidyr-example.txt) and can be imported using `read.delim`
...
- the *names* of the columns in the data frame that are to be collapsed
    + note you don't need quote marks (`""`) to define the column names
...
Non R-users probably find these commands a bit obtuse, and there are many points that can trip-up beginner users

- why the `[ ]`?
- what is `c(..)`?
- `Name` and `Race` need to be in quote marks
...
```{r}
select(patients, contains("Grade"))
```

see also

- starts_with, matches, ends_with

******
## Exercise

- Print all columns between `Height` and `Grade_Level`
- Print all columns that contain "eight"
- Print all columns between `Height` and `Grade_Level`, but NOT `Pet`
...
`mutate` allows us to compute and append one or more new columns to a data frame. We do not need to use the `$` operator to extract variables, we can refer to them by name.

```{r}
mutate(patients, Sex = str_trim(Sex))
```

We can create a new, cleaned, data frame and overwrite the Sex column
...
It also supports some more sophisticated manipulations, such as being able to subtract and add times.
