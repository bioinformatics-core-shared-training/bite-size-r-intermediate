...
The `iris` data frame is a [classic dataset](https://en.wikipedia.org/wiki/Iris_flower_data_set) that is built into R. To load these data, we use the `data` function with the name `iris` (notice no quotation marks) in brackets. The data frame can then be printed to screen by typing it's name (`iris`). This dataset is also a good example of data in the tidy form.
...
- Open [Rmarkdown template](2.dplyr-intro-exercise.Rmd)
...
- See [Rmarkdown template](2.dplyr-intro-exercise.Rmd)
