...
Read a cleaned version of the patients dataset (patient-data-cleaned.txt) into R and use `summarise`, `summarise_each`, `mutate_each` and `group_by` to answer the following questions:
...
* Find the number of visits made by each patient to the hospital to have blood glucose and pressure measurements taken (_hint: use the `n` function from `dyplr`, type `?n` for help_).

* Find the date of the last visit made by each patient (_hint: use `arrange` to sort by date and the `last` function to select the last one_).
