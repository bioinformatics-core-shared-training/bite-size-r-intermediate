...
# comment out for condensed script
...
Note the packages that are loaded, relate these to this and other sections of the course.

#### Aside: function name conflicts (optional)

Note the conflicts and briefly discuss the problem with the same function names being used
by multiple packages and what you have to do to use a function that has been masked by another
package.
...
mappings in ggplot function instead.
...
We can tell ggplot to fit the subplots onto a single row.
...
Take a look at the x and y scales in the above plot. ggplot has chosen the x and y
...
First, we'll change the breaks, i.e. where ggplot puts ticks and numeric labels, on the
y axis.
...
ggplot can provides a set of colour palettes from [ColorBrewer]( http://colorbrewer2.org).
...
ggplot displays categories in the order given by the levels of the factor. If a character
variable is presented to ggplot as if it were a factor, ggplot converts it into a factor
