...
```{r}
patients <- mutate_at(patients, vars(Sex, Smokes, State, Pet, Grade, Age), funs(factor))
patients
```
...
ggplot(data = patients, mapping = aes(x = Height, y = Weight)) +
  geom_point() +
  facet_wrap(~ Sex + Smokes + Overweight)
```
...
```{r}
ggplot(data = patients, mapping = aes(x = Height, y = Weight, colour = Pet)) +
  geom_point() +
...
```{r}
pet_counts <- count(patients, Pet)
pet_counts
ggplot(data = pet_counts, mapping = aes(x = Pet, y = n)) +
...
```{r}
ggplot(data = patients, mapping = aes(x = Pet)) +
  geom_bar(stat = "count")
```

By default `geom_bar` applies the statistical transformation, `stat_count`, in
much the same way as we did above to create the `pet_counts` summary table.
